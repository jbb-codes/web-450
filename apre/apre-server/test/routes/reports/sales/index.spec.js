...
// Tests for the sales by category endpoints
describe("Apre Sales Report API - Categories", () => {
  beforeEach(() => {
    mongo.mockClear();
  });

  // Test 1: should fetch a list of distinct sales categories
  it("should fetch a list of distinct sales categories", async () => {
    mongo.mockImplementation(async (callback) => {
      const db = {
        ...
        distinct: jest
          .fn()
          .mockResolvedValue(["Electronics", "Clothing", "Food"]),
      };
      await callback(db);
    });
    ...
    expect(response.status).toBe(200);
    expect(response.body).toEqual(["Electronics", "Clothing", "Food"]);
  });

  // Test 2: should fetch sales data for a specific category, grouped by salesperson
  it("should fetch sales data for a specific category, grouped by salesperson", async () => {
    mongo.mockImplementation(async (callback) => {
      const db = {
        collection: jest.fn().mockReturnThis(),
        aggregate: jest.fn().mockReturnValue({
          toArray: jest.fn().mockResolvedValue([
            { salesperson: "Alice Johnson", totalSales: 2500 },
            { salesperson: "Bob Smith", totalSales: 1800 },
          ]),
        }),
      };
      await callback(db);
    ...
    const response = await request(app).get(
      "/api/reports/sales/categories/Electronics",
    );
  ...
  // Test 3: should return 200 with an empty array if no sales data found for the category
  it("should return 200 with an empty array if no sales data is found for the category", async () => {
    mongo.mockImplementation(async (callback) => {
      const db = {
        collection: jest.fn().mockReturnThis(),
        aggregate: jest.fn().mockReturnValue({
    ...
    const response = await request(app).get(
      "/api/reports/sales/categories/unknown-category",
    );
